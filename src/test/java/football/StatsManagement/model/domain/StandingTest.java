...
//  }
  ...
  void initialStanding() throws ResourceNotFoundException, IOException {
    ...
    try (FileWriter writer = new FileWriter("build/reports/tests/test/error.log")) {
      ...
      writer.write("Arrange: クラブを準備しました\n");

      when(service.getClubsByLeague(leagueId)).thenReturn(clubs);
      writer.write("Arrange: サービスからリーグのクラブを取得しました\n");

      when(service.getLeague(leagueId)).thenReturn(new League(leagueId, 1, "league1"));
      writer.write("Arrange: サービスからリーグを取得しました\n");

      when(service.getSeason(seasonId)).thenReturn(new Season(seasonId, "1000-01", LocalDate.of(1000, 7, 1), LocalDate.of(1000, 6, 30), false));
      writer.write("Arrange: サービスからシーズンを取得しました\n");
      ...
      writer.write("Arrange: クラブの順位を準備しました\n");
      ...
      try (MockedStatic<ClubForStanding> mockedClubForStanding = mockStatic(ClubForStanding.class);
        ...
        // Assert
        assertEquals(expected, actual);
        writer.write("Assert: 期待される順位表と実際の順位表が一致しました\n");
        ...
        writer.write("Assert: モックの検証を完了しました\n");
      } catch (Exception e) {
        writer.write("エラー発生: " + e.getMessage() + "\n");
      }
    } // try-with-resourcesでFileWriterを閉じる
  }
}